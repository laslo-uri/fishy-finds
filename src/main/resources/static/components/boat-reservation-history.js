Vue.component('boat-reservation-history', {
data: function(){
    		return{
    			showPage: 0,
    			sortOption: "",
...
<div class="ff-catalog">
            ...
            <section class="ff-catalog__shell">
                <div class="ff-section-head">
                    <h2>Boat reservation history</h2>
                    <p>Past boat trips - search, sort, filter, and leave feedback.</p>
                </div>
                ...
                <div v-show="showPage == 0">
                    ...
                    <p v-if="!reservations.length" class="ff-empty">No boat reservations found.</p>
                </div>
                ...
                <div v-show="showPage == 1">
                    <div class="ff-detail">
                        ...
                        <h3 class="ff-detail__title">{{ boatToShow.offerName }}</h3>
                        <div class="ff-detail__grid">
                            <div><span>Country</span><strong>{{ boatToShow.location && boatToShow.location.country }}</strong></div>
                            <div><span>City</span><strong>{{ boatToShow.location && boatToShow.location.city }}</strong></div>
                            <div><span>Street</span><strong>{{ boatToShow.location && boatToShow.location.street }} {{ boatToShow.location && boatToShow.location.streetNumber }}</strong></div>
                            <div><span>Price</span><strong>{{ boatToShow.unitPrice }}</strong></div>
                            <div><span>Capacity</span><strong>{{ boatToShow.maxCustomerCapacity }}</strong></div>
                            <div class="ff-detail__wide"><span>Description</span><strong>{{ boatToShow.description }}</strong></div>
                            <div class="ff-detail__wide"><span>Services</span><strong>{{ additionalServices }}</strong></div>
                            <div class="ff-detail__wide"><span>Rules</span><strong>{{ boatToShow.rulesOfConduct }}</strong></div>
                            <div class="ff-detail__wide"><span>Cancellation</span><strong>{{ boatToShow.cancellationPolicy }}</strong></div>
                        </div>
                    </div>
                </div>
                ...
                <div v-show="showPage == 2">
                    <div class="ff-detail">
                        ...
                        <h3 class="ff-detail__title">Feedback for boat and owner</h3>
                        <div class="ff-form-grid">
                            <input type="text" class="ff-field" disabled :value="boatToShow.offerName" placeholder="Boat name" />
                            <input type="text" class="ff-field" disabled :value="boatToShow.user && boatToShow.user.firstName" placeholder="Owner first name" />
                            <input type="text" class="ff-field" disabled :value="boatToShow.user && boatToShow.user.lastName" placeholder="Owner last name" />
                            <input type="number" class="ff-field" placeholder="Rating for owner (1-5)" v-model="feedback.rateForOwner" min="1" max="5" />
                            <input type="number" class="ff-field" placeholder="Rating for boat (1-5)" v-model="feedback.rateForOffer" min="1" max="5" />
                            <textarea rows="3" class="ff-field ff-field--wide" placeholder="Feedback for owner" v-model="feedback.contentForOwner"></textarea>
                            <textarea rows="3" class="ff-field ff-field--wide" placeholder="Feedback for boat" v-model="feedback.contentForOffer"></textarea>
                        </div>
          ...
          methods : {
            showMore : function(boat){
               this.boatToShow = boat.offer || this.boatToShow;
               if (!this.boatToShow.location) {
                 this.boatToShow.location = { country: '', city: '', street: '', streetNumber: '' };
               }
               this.additionalServices = boat.additionalServices || '';
               this.showPage = 1;
            },
            showFeedback : function(bung){
                this.boatToShow = bung.offer || this.boatToShow;
                if (!this.boatToShow.location) {
                  this.boatToShow.location = { country: '', city: '', street: '', streetNumber: '' };
                }
